...
## Wiring it up — `sitemapConnector()` and `src/config/sitemap.ts`

`warlock add sitemap` (`requires: ["web"]`) writes both halves. Doing it by
hand is the same two pieces:
...
request-derived host — then registers `GET <path>`. The route itself
re-reads the page graph on every request via `listRoutablePages()`, not once
at boot, so it never goes stale under `warlock dev`.
...
- `sitemapConnector(options?)` (`src/sitemap-connector.ts`) — the connector
  `warlock.config.ts` registers; see "Wiring it up" above.
